...
const DataVis = () => {
  ...
  const fetchGlobalDateRange = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/answer/history/aggregated");
  ...
  const fetchQTypes = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/qtype/all");
      if (res.data.success) {
  ...
  const fetchAggregatedData = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get("http://localhost:5000/api/answer/history/aggregated", { params });
      if (res.data.success) {
