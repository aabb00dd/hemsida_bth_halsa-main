...
test("POST /question/validate returns incorrect for wrong answer", async () => {
  const res = await request(app)
    .post("/question/validate")
    .send({
      question_id: 1,
      question_data_index: 0,
      user_answer: "wrongname",
      user_uid: "test-user",
      timestamp: "2025-05-12T08:40:00Z"
    });

  expect(res.statusCode).toBe(200);
  expect(res.body.correct).toBe(false);
  expect(res.body.expected).toBeDefined();
});

test("Answer is logged in answer_submissions", (done) => {
  db.get("SELECT * FROM answer_submissions WHERE user_uid = ?", ["test-user"], (err, row) => {
    expect(err).toBeNull();
    expect(row).toBeDefined();
    expect(row.user_answer).toBeDefined();
    expect(row.correct).not.toBeNull();
    done();
  });
});
